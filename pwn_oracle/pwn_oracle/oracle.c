...
int main() {
    ...
    if (setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, &socket_reuseaddr, sizeof(socket_reuseaddr)) == -1) {
        perror("setsockopt");
        exit(1);
    }
...
// Aqui no hay vulns solo son mensajes en base a VIEW
void handle_view() {
...
void handle_plague() {
    ...
    char *plague_target = (char *)0x0; // 0x0 como primer elemento para evitar overwrite
...
void parse_headers() {
    ...
    // Vuln: Aqui esta el overflow, ya que es i no parara de incrementar hasta que haya un \r\n\r\n
    while (1) {
        recv(client_socket, &byteRead, sizeof(byteRead), 0);
...
int is_competitor(char *name) {
    ...
    if (!strncmp(name, "Overlord", 8)) // No es vulnerable
        return 0;
